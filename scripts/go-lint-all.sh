...
LINT_TAGS="ledger,test_ledger_mock,sims"
export LINT_TAGS
...
if [[ -z "${LINT_DIFF:-}" ]]; then
  # Full lint: exclude api and orm modules
  find "${REPO_ROOT}" -type f -name go.mod \
    -not -path "${REPO_ROOT}/api/go.mod" \
    -not -path "${REPO_ROOT}/orm/go.mod" \
    -print0 |
