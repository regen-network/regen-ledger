...
**Cosmovisor 1.1 with the auto-download option disabled is required. Node operators must prepare the upgrade binary in advance.**

::: warning
The auto-download option is not supported in Cosmovisor v1.1 when upgrading from Regen Ledger 1.0 (because it uses Cosmos SDK v0.42). Enabling the auto-download option will result in an error and a failed restart at the time of the upgrade.
:::
