...
Create the folder for the upgrade binary (`v3.0.0`) and copy the `regen` binary into the folder. This next step assumes that the `regen` binary was built from source and stored in the `build` directory (this step is not required if the auto-download option was enabled):
