...
This document provides instructions for performing an IBC token transfer using either [Keplr Wallet](https://wallet.keplr.app/) or the [command-line interface (CLI)](../infrastructure/interfaces.html#command-line-interface). 
...
IBC transfers are also possible using the [command-line interface (CLI)](../infrastructure/interfaces.html#command-line-interface). For instructions on how to install the `regen` binary, check out [Quick Start](../get-started/).
