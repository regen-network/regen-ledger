...
Create a `v7.0` directory and copy the upgrade binary (`v7.0.0`) to the directory:

```bash
mkdir -p $HOME/.regen/cosmovisor/upgrades/v7.0/bin
cp ./build/regen $HOME/.regen/cosmovisor/upgrades/v7.0/bin
```
Ensure the right `regen` binary has been placed in the directory:

```bash
$HOME/.regen/cosmovisor/upgrades/v7.0/bin/regen version
```
