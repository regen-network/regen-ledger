...
This document provides guidelines for node operators upgrading from Regen Ledger v4.x to v5.0. For a general overview of the upgrade process, please refer to [Upgrade Overview](README.md).
...
At the proposed block height, `cosmovisor` will automatically stop the current binary (`v4.x`), set the upgrade binary as the current binary (`v5.0.0`), and then (depending on the cosmovisor settings) perform a backup and restart the node.
