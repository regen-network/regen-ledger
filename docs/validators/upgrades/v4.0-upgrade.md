...
Either download the upgrade binary (`v4.0.0`) or build it from source:
...
Create the folder for the upgrade binary (`v4.0.0`) and copy the `regen` binary into the folder. This next step assumes that the `regen` binary was built from source and stored in the `build` directory:

```bash
mkdir -p $HOME/.regen/cosmovisor/upgrades/v4.0.0/bin
cp $HOME/regen-ledger/build/regen $HOME/.regen/cosmovisor/upgrades/v4.0.0/bin
```

At the proposed block height, `cosmovisor` will automatically stop the current binary (`v3.0.0`), set the upgrade binary as the new current binary (`v4.0.0`), and then restart the node.
