...
func TestMigrations(t *testing.T) {
	...
	expbd1, err := core.FormatBatchDenom("C0101", 1, &startDate, &endDate)
	require.NoError(t, err)
	expbd2, err := core.FormatBatchDenom("C0102", 1, &startDate, &endDate)
	require.NoError(t, err)
	expbd3, err := core.FormatBatchDenom("C0102", 2, &startDate, &endDate)
	require.NoError(t, err)
