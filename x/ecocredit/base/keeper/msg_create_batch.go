...
func (k Keeper) CreateBatch(ctx context.Context, req *types.MsgCreateBatch) (*types.MsgCreateBatchResponse, error) {
	...
	for _, issuance := range req.Issuance {
		...
		if err = sdkCtx.EventManager().EmitTypedEvent(&types.EventMint{
			BatchDenom:     batchDenom,
	...
	if err = sdkCtx.EventManager().EmitTypedEvent(&types.EventCreateBatch{
		BatchDenom: batchDenom,
	...
	return &types.MsgCreateBatchResponse{BatchDenom: batchDenom}, nil
}
