...
func TestQueryBatchesByIssuer(t *testing.T) {
	...
	assert.NilError(t, s.stateStore.BatchTable().Insert(s.ctx, &api.Batch{Issuer: s.addr, BatchDenom: "1"}))
	assert.NilError(t, s.stateStore.BatchTable().Insert(s.ctx, &api.Batch{Issuer: s.addr, BatchDenom: "2"}))
	assert.NilError(t, s.stateStore.BatchTable().Insert(s.ctx, &api.Batch{Issuer: otherAddr, BatchDenom: "3"}))
