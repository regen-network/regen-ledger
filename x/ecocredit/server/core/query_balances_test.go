...
func TestQuery_Balances(t *testing.T) {
	...
	balance1 := &ecocreditv1.BatchBalance{Address: s.addr, BatchId: 1, Tradable: "15", Retired: "15", Escrowed: "15"}
	balance2 := &ecocreditv1.BatchBalance{Address: s.addr, BatchId: 2, Tradable: "19", Retired: "20", Escrowed: "33"}
	...
	assert.NilError(t, s.stateStore.BatchBalanceTable().Insert(s.ctx, &ecocreditv1.BatchBalance{
		Address:  s.addr,
		BatchId:  3,
		Tradable: "4",
		Retired:  "5",
...
func assertBalanceEqual(t *testing.T, balance *core.BatchBalance, batchBalance *ecocreditv1.BatchBalance) {
