...
func TestQueryAllBalances(t *testing.T) {
	...
	bal1 := res.Balances[0]
	bal2 := res.Balances[1]
	assert.Equal(t, bal1.Address, addr2.String())
	assert.Equal(t, bal2.Address, addr1.String())

	// totals will be balances added from batch1 and batch2.
	assert.Equal(t, bal2.Balances.TradableAmount, "16")
	assert.Equal(t, bal2.Balances.RetiredAmount, "17")
	assert.Equal(t, bal2.Balances.EscrowedAmount, "18")
	assert.Equal(t, len(bal2.Balances.BatchDenoms), 2)

	// should only have 1 balance
	assert.Equal(t, bal1.Balances.BatchDenoms[0], "C02-001-20200101-20220101-001")
	assert.Equal(t, bal1.Balances.TradableAmount, "19")
	assert.Equal(t, bal1.Balances.RetiredAmount, "20")
	assert.Equal(t, bal1.Balances.EscrowedAmount, "33")
}
