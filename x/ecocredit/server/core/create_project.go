...
func (k Keeper) CreateProject(ctx context.Context, req *core.MsgCreateProject) (*core.MsgCreateProjectResponse, error) {
	...
	if projectID == "" {
		exists := true
		for exists {
			projectID, err = k.genProjectID(ctx, classInfo.Key)
			if err != nil {
				return nil, err
			}
...
func (k Keeper) genProjectID(ctx context.Context, classKey uint64) (string, error) {
	var nextSeq uint64
	projectSeq, err := k.stateStore.ProjectSequenceTable().Get(ctx, classKey)
	switch err {
	case ormerrors.NotFound:
		nextSeq = 1
	case nil:
		nextSeq = projectSeq.NextSequence
	default:
		return "", err
	}
	...
	return ecocredit.FormatProjectID(nextSeq), nil
}
