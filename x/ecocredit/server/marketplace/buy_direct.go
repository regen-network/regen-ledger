...
func (k Keeper) BuyDirect(ctx context.Context, req *marketplace.MsgBuyDirect) (*marketplace.MsgBuyDirectResponse, error) {
	...
	if req.PricePerCredit.Denom != market.BankDenom {
		return nil, sdkerrors.ErrInvalidRequest.Wrapf("price per credit denom does not match ask price denom: "+
			" %s, expected %s", req.PricePerCredit.Denom, market.BankDenom)
	}
	...
	if err = k.fillOrder(ctx, sellOrder, buyerAcc, creditOrderQty, coinCost, false, !req.DisableAutoRetire, req.RetirementLocation, batch.BatchDenom); err != nil {
