...
func (k Keeper) BuyDirect(ctx context.Context, req *marketplace.MsgBuyDirect) (*marketplace.MsgBuyDirectResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sell order %d: %w", req.SellOrderId, err)
	}
	...
	creditOrderQty, err := math.NewPositiveFixedDecFromString(req.Quantity, ct.Precision)
	if err != nil {
		return nil, err
	}
	...
	if sellOrderPricePerCredit.GT(req.BidPrice.Amount) {
		return nil, sdkerrors.ErrInvalidRequest.Wrapf("price per credit too low: sell order ask per credit: %v, request: %v", sellOrderPriceCoin, req.BidPrice)
	}

	// check address has the total cost (price per * order quantity)
	bal := k.bankKeeper.GetBalance(sdkCtx, buyerAcc, req.BidPrice.Denom)
	...
	if bal.IsLT(coinCost) {
		return nil, sdkerrors.ErrInsufficientFunds.Wrapf("requested to purchase %s credits @ %s%s per "+
			"credit (total %v) with a balance of %v", req.Quantity, sellOrder.AskPrice, market.BankDenom, coinCost, bal)
	}

	// fill the order, updating balances and the sell order in state
	if err = k.fillOrder(ctx, sellOrder, buyerAcc, creditOrderQty, coinCost, false, !req.DisableAutoRetire,
		req.RetirementLocation, batch.BatchDenom); err != nil {
