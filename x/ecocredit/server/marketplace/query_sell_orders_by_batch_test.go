...
func TestSellOrdersByBatch(t *testing.T) {
	...
	res, err := s.k.SellOrdersByBatch(s.ctx, &marketplace.QuerySellOrdersByBatchRequest{
	...
	_, err = s.k.SellOrdersByBatch(s.ctx, &marketplace.QuerySellOrdersByBatchRequest{
		BatchDenom: "yikes!",
		Pagination: nil,
	})
