...
func TestSell_Valid(t *testing.T) {
	...
	supplyBefore, err := s.coreStore.BatchSupplyStore().Get(s.ctx, 1)
	assert.NilError(t, err)
	...
	balanceAfter, err := s.coreStore.BatchBalanceStore().Get(s.ctx, s.addr, 1)
	assert.NilError(t, err)
	supplyAfter, err := s.coreStore.BatchSupplyStore().Get(s.ctx, 1)
	assert.NilError(t, err)
...
func TestSell_Prune(t *testing.T) {
	...
	_, err = s.marketStore.SellOrderStore().Get(s.ctx, shouldBeExpired)
	assert.ErrorContains(t, err, ormerrors.NotFound.Error())

	_, err = s.marketStore.SellOrderStore().Get(s.ctx, shouldBeValid)
	assert.NilError(t, err)
}
...
func testSellSetup(t *testing.T, s *baseSuite, batchDenom, bankDenom, displayDenom, classId string, start, end *timestamppb.Timestamp, creditType ecocredit.CreditType) {
	...
	assert.NilError(t, s.k.coreStore.BatchSupplyStore().Insert(s.ctx, &ecocreditv1.BatchSupply{
		BatchId:  1,
		TradableAmount: "100",
		RetiredAmount:  "100",
	}))
