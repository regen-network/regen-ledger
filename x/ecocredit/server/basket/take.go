...
func (k Keeper) Take(ctx context.Context, msg *baskettypes.MsgTake) (*baskettypes.MsgTakeResponse, error) {
	...
	err = k.bankKeeper.SendCoinsFromAccountToModule(sdkContext, acct, ecocredit.ModuleName, basketCoins)
	if err != nil {
		return nil, err
	}

	err = k.bankKeeper.BurnCoins(sdkContext, ecocredit.ModuleName, basketCoins)
	if err != nil {
		return nil, err
	}
