...
func ExpectParamGet(obj interface{}, paramKeeper *mocks.MockParamKeeper, times int) {
	...
	case *[]string:
		s := obj.(*[]string)
		paramKeeper.EXPECT().Get(gmAny, gmAny, gmAny).Do(func(_, _ any, param *[]string) {
			*param = *s
		}).Times(times)
	case *sdk.Coins:
		coins := obj.(*sdk.Coins)
		paramKeeper.EXPECT().Get(gmAny, gmAny, gmAny).Do(func(_, _ any, param *sdk.Coins) {
			*param = *coins
		}).Times(times)
	case *bool:
		b := obj.(*bool)
		paramKeeper.EXPECT().Get(gmAny, gmAny, gmAny).Do(func(_, _ any, param *bool) {
			*param = *b
		}).Times(times)
	case *[]*core.AskDenom:
		askDenoms := obj.(*[]*core.AskDenom)
		paramKeeper.EXPECT().Get(gmAny, gmAny, gmAny).Do(func(_, _ any, param *[]*core.AskDenom) {
			*param = *askDenoms
		}).Times(times)
	}
}
