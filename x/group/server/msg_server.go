...
func (s serverImpl) CreateGroup(ctx types.Context, req *group.MsgCreateGroupRequest) (*group.MsgCreateGroupResponse, error) {
	...
	maxMetadataSize := s.maxMetadataSize(ctx)
	if err := assertMetadataSize(metadata, maxMetadataSize, "group metadata"); err != nil {
		return nil, err
	}
	...
	for i := range members {
		m := members[i]
		if err := assertMetadataSize(m.Metadata, maxMetadataSize, "member metadata"); err != nil {
			return nil, err
		}
...
func (s serverImpl) CreateGroupAccount(ctx types.Context, req *group.MsgCreateGroupAccountRequest) (*group.MsgCreateGroupAccountResponse, error) {
	...
	if err := assertMetadataSize(metadata, s.maxMetadataSize(ctx), "group account metadata"); err != nil {
		return nil, err
	}
...
func (s serverImpl) CreateProposal(ctx types.Context, req *group.MsgCreateProposalRequest) (*group.MsgCreateProposalResponse, error) {
	...
	if err := assertMetadataSize(metadata, s.maxMetadataSize(ctx), "metadata"); err != nil {
		return nil, err
	}
...
// maxMetadataSize returns the maximum length of a metadata
func (s serverImpl) maxMetadataSize(ctx types.Context) int {
	var result uint32
	s.paramSpace.Get(ctx.Context, group.ParamMaxCommentLength, &result)
	return int(result)
}

func assertMetadataSize(metadata []byte, maxMetadataSize int, description string) error {
