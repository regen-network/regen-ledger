...
func DoExecuteMsgs(ctx sdk.Context, msgServiceRouter *baseapp.MsgServiceRouter, router sdk.Router, groupAccount sdk.AccAddress, msgs []sdk.Msg) ([]sdk.Result, error) {
	...
	for i, msg := range msgs {
		if svcMsg, ok := msg.(sdk.ServiceMsg); ok {
			...
			if handler == nil {
				return nil, errors.Wrapf(group.ErrInvalid, "unrecognized message service method: %s; message index: %d", msgFqName, i)
			}
