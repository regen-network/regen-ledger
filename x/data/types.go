...
func (ch ContentHash) Validate() error {
	...
	default:
		return sdkerrors.ErrInvalidRequest.Wrap(fmt.Sprintf("invalid %T type %T", ch, hash))
	}
}
...
func (x MediaType) Validate() error {
	if _, ok := MediaType_name[int32(x)]; !ok {
		return sdkerrors.ErrInvalidRequest.Wrap(fmt.Sprintf("unknown %T %d", x, x))
	}
...
func (x DigestAlgorithm) Validate(hash []byte) error {
	...
	if !ok {
		return sdkerrors.ErrInvalidRequest.Wrap(fmt.Sprintf("invalid or unknown %T %s", x, x))
	}

	nBytes := nBits / 8
	if len(hash) != nBytes {
		return sdkerrors.ErrInvalidRequest.Wrap(fmt.Sprintf("expected %d bytes for %s, got %d", nBytes, x, len(hash)))
	}
...
func (x GraphCanonicalizationAlgorithm) Validate() error {
	...
	if x == GraphCanonicalizationAlgorithm_GRAPH_CANONICALIZATION_ALGORITHM_UNSPECIFIED {
		return sdkerrors.ErrInvalidRequest.Wrap(fmt.Sprintf("invalid %T %s", x, x))
	}
