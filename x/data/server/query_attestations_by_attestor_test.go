...
func TestQuery_AttestationsByAttestor(t *testing.T) {
	...
	require.EqualError(t, err, "rpc error: code = InvalidArgument desc = attestor cannot be empty")

	// query attestations with invalid attestor address
	_, err = s.server.AttestationsByAttestor(s.ctx, &data.QueryAttestationsByAttestorRequest{
		Attestor: "foo",
	})
	require.EqualError(t, err, "rpc error: code = InvalidArgument desc = attestor: decoding bech32 failed: invalid bech32 string length 3")
}
