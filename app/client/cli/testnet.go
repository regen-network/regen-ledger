...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", sdk.DefaultBondDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01uregen)")
...
Example:
	regen testnet init-files --v 4 --output-dir ./.localnet --starting-ip-address 192.168.10.2
	`,
...
Example:
	regen testnet --v 4 --output-dir ./.localnet
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		defaultPowerReduction := sdkMath.NewInt(sdk.DefaultPowerReduction.Int64())

		accTokens := sdk.TokensFromConsensusPower(1000, defaultPowerReduction)
		accStakingTokens := sdk.TokensFromConsensusPower(500, defaultPowerReduction)
		coins := sdk.Coins{
		...
		valTokens := sdk.TokensFromConsensusPower(100, defaultPowerReduction)
...
func startTestnet(cmd *cobra.Command, args startArgs) error {
	networkConfig := network.DefaultConfig(testsuite.NewTestNetworkFixture)
