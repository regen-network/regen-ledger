...
func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *XrnApp) (
...
func testAndRunTxs(app *XrnApp) []simulation.WeightedOperation {
...
func invariants(app *XrnApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/cosmos/cosmos-sdk/XrnApp -bench ^BenchmarkFullAppSimulation$ -SimulationCommit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	app := NewXrnApp(logger, db, nil, true, 0)
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := NewXrnApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "XrnApp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewXrnApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "XrnApp", newApp.Name())
